# ClearUI C10 modifications: banked named groups, bounded transfers, receive-only channels.
...
DRIVER_VERSION = "Fusion 6.0.0 ClearUI C10: multiboot, named groups and receive-only"
...
CHANNELDISP_LIST = ["Frequency only", "Frequency large + name", "Name only", "Name large + frequency"]
...
# Welcome list
WELCOME_LIST = ["Message line 1, Voltage, Sound (ALL)", "Make 2 short sounds (SOUND)", "User message line 1 and line 2 (MESSAGE)", "Battery voltage (VOLTAGE)", "ClearUI logo", "NONE"]
...
# ClearUI virtual D000..D200 maps to external flash 00D000..00D200 in the active configuration bank.
MEM_SIZE =      0x00D200
CAL_END =       0x00B190
...
# Persisted EEPROM action IDs: keep this order aligned with ACTION_OPT_t.
...
def do_download(radio):
    ...
    if f:
        if f != "ClearUI C10":
            raise errors.RadioError("This driver requires ClearUI C10 firmware; detected " + f)
...
def do_upload(radio):
    ...
    if f != "ClearUI C10":
        ...
        if any(raw[a+15] == 0xA5 and raw[a:a+4] not in (b"\xff"*4, b"\x00"*4) for a in addresses):
            raise errors.RadioError("Receive-only channels require ClearUI C10 firmware; flash C10 before uploading this image")
...
class UVK5RadioEgzumer(chirp_common.CloneModeRadio):
    ...
    VENDOR = "Quansheng"
    MODEL = "UV-K1 & UV-K5 V3 (ClearUI Multiboot C10)"
    _memsize = MEM_SIZE
    BAUD_RATE = 38400
...
# advanced settings too
    ...
    def _get_scanlist_name(self, index):
        ...
        try:
            name_obj = self._memobj.clearui.list[index].name
...
#                mem.extra.append(rs)
            ...
            mem.extra.append(RadioSetting("receiveOnly", "Receive only", RadioSettingValueBoolean(False)))
        ...
        rs = RadioSetting("receiveOnly", "Receive only", val)
        rs.set_doc("Disables transmitting regardless of band unlock or TX Lock; requires ClearUI C10")
        mem.extra.append(rs)
...
#                                "in the range %.1f - %.1f" % (FMMIN , FMMAX))
            ...
            if elname == "slDef":
                ...
                if 0 <= idx < (MR_CHANNELS_LIST - 1):
                    name = str(element.value).encode('ascii')[:16].rstrip(b' ')
...
#        dtmfc = RadioSettingGroup("dtmfc", "DTMF Contacts")
        ...
        def get_action(action_num):
            ...
            if action_num < 0 or action_num >= len(KEYACTIONS_LIST):
                action_num = 0
            return KEYACTIONS_LIST, KEYACTIONS_LIST[action_num]
        ...
        for i in range(MR_CHANNELS_LIST - 1):
            listname = self._get_scanlist_name(i)
            ...
            listname_setting.set_doc(
                ...
                f'Maximum 16 ASCII characters; requires ClearUI firmware'
            )
    ...
    # basic.append(listname_setting)
        ...
        SetMetSetting = RadioSetting("set_met", "Meter style (Spine / Ribbon)", val)
        SetMetSetting.set_doc('Spine uses a vertical meter; Ribbon uses a horizontal strip.')
        # Set_gui f4hwn
        ...
        bat_txt_setting.set_doc('Icon, Voltage, Percentage, or Icon + percentage.')
...
#        _mem_attr.is_free = 0
        ...
        if number < MR_CHANNELS_MAX:
            _mem_chname = self._memobj.channelname[number]
            tag = memory.name[:16].ljust(16, "\x00")
